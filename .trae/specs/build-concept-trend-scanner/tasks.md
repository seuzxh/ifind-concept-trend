...
- [x] Task 4: 多因子综合评分引擎
  ...
  - [x] 4.2 实现涨幅因子（(最后 close - 昨收) / 昨收 × 100%）
  - [x] 4.3 实现实体涨幅因子（(最后 close - 第1根 open) / 第1根 open × 100%）
  - [x] 4.4 实现成交额因子（5 根 K 线 amount 求和）
  - [x] 4.5 实现量比因子（从 kline_1min 获取最后一根 K 线的 LB 值）
  - [x] 4.6 实现加权综合评分公式（权重可配置：涨幅 0.25、实体涨幅 0.30、成交额 0.20、量比 0.25）
  - [x] 4.7 实现强势个股标记（涨幅 > 7% 或实体涨幅 > 5% 标记为"强势"）
  - [x] 4.8 实现概念板块评分（板块强势得分 = 强势个股占比 × 60% + 板块平均得分 × 40%）
  - [x] 4.9 实现评分结果写入 scan_result 表（stock_daily_scan、board_daily_scan）
  - [x] 4.10 编写评分引擎单元测试
