...
- 新建结果推送模块：通过飞书 webhook 推送文本格式扫描报告
- 新建 CLI 入口：提供命令行交互界面
...
- **AND** 返回字段含义：
  ...
  - `p03798_f012`: 涨跌幅（%）
  - `p03798_f016`: 区间开始日期
- **AND** 每个板块按涨跌幅（p03798_f012）降序取 Top30，剔除 ST 股
- **AND** 所有板块的个股合并去重，作为 T 日观察股池
...
- **AND** 仅对缺失数据的股票调用 `POST /api/v1/high_frequency`
  ...
  - functionpara.Fill: `Original`（不填充缺失K线）
  - functionpara.calculate: `{"LB": "5"}`（5日量比计算参数）
